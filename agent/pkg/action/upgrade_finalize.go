...
// NewFinalizeContextInline builds a context for running RunFinalize on the
// host as a fallback: no /host prefix, ImgMountPoint is the mount point of
// the transition image, everything else is a real host path.
func NewFinalizeContextInline(spec *v1.UpgradeSpec, upgradeImg *sdkImages.Image) FinalizeContext {
	ctx := FinalizeContext{
...
func NewFinalizeContextForHandoff(spec *v1.UpgradeSpec, upgradeImg *sdkImages.Image, hostPrefix string) FinalizeContext {
	ctx := NewFinalizeContextInline(spec, upgradeImg)
	ctx.ImgMountPoint = "/"
...
// RunFinalize runs the post-deploy upgrade finalize steps: label state
// images, create rootfs extra dirs, SELinux relabel the target rootfs, run
// the after-upgrade-chroot hook, rebrand the default GRUB entry and refresh
// the ESP. This is the set of steps the target image's code owns; running
// them from the target agent, rather than the possibly-older host agent, is
// the point of the chroot handoff introduced with the upgrade-finalize
// subcommand. Called either from that subcommand inside the target chroot,
// or inline from the host as a fallback for images that predate the
// subcommand.
