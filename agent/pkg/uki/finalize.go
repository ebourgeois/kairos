...
// kairos-uki-upgrade.after yip stage and hook script.
//
// These are the same steps UpgradeAction.Run used to run inline; moving
// them here means when the host chroots-out to the target's kairos-agent
// (agent binary extracted from the signed .efi's .initrd; see
// ExtractFromInitrd), the target's newer code runs them, so a format
// change in loader/entries/*.conf or the EFI key layout ships in the
// image rather than needing every host agent released before it to
// already understand the new shape.
//
